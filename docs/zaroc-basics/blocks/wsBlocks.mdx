...
> - Button block is every component that uses buttons, you can use image, text, documents, videos on header.
...
```php
$wsHeaderType = ["image"];                            // image - text - document - video on header
$wsHeaderContent = ["$welcomeImg"];                   // text -  image/document/video url with extension
